...
public interface BakeryRepository extends JpaRepository<BakeryVO, String> {
	...
	@Query(value = "SELECT SQRT(POWER(LATITUDE - ?1, 2) + POWER(LONGITUDE - ?2, 2)) AS DISTANCE, BAKERY.* FROM BAKERY WHERE STORENAME LIKE %?3% ORDER BY DISTANCE", nativeQuery = true)
